...
class NewsOptions extends StatefulWidget {
  ...
  @override
  State<NewsOptions> createState() => _NewsOptionState();
}

class _NewsOptionState extends State<NewsOptions> {

  List<News>? allItems =[];
  List<News_Single> list = [];
  bool isLoaded = false;
  ...
  Future<void> _getAllItems() async {
    allItems = await RemoteService().getNews();
    list.clear();
    setState(() {
      ...
      for (int i = 0; i < allItems!.length; i++) {
        ...
        print("$link");
        list.add(News_Single(allItems![i].id,allItems![i].name,images,allItems![i].youtubelink));
      }
      isLoaded = true;
    });
  }
